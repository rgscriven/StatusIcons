waitUntil {!(isNull (findDisplay 46))};
	...
	sb_checkTemp = {
			...
			if ( (_bodyTemperature< 36) && ((player getVariable "sb_isCold") isEqualTo false) ) then {
				...
				(_display displayCtrl 13375) ctrlSetText "Addons\StatusIcons\icons\cold.paa";
	...
	sb_updateIcons = {
		...
		if (_iconh != "-1") then {
			(_display displayCtrl 13392) ctrlSetText format["Addons\StatusIcons\circlebar\blood\%1.paa",_iconh];
			[(_display displayCtrl 13392)] call sb_fadeIn;
		};
		
		if (_iconf != "-1") then {
			(_display displayCtrl 13393) ctrlSetText format["Addons\StatusIcons\circlebar\hunger\%1.paa",_iconf];
			[(_display displayCtrl 13393)] call sb_fadeIn;
		};
		
		if (_iconw != "-1") then {
			(_display displayCtrl 13394) ctrlSetText format["Addons\StatusIcons\circlebar\thirst\%1.paa",_iconw];
	...
	sb_init = {
		...
		_initIcons = {
			...
			_display = (uiNamespace getVariable "StatusIcons");
			_ctrl = _display displayCtrl _idc;
			_ctrl ctrlSetText format["Addons\StatusIcons\icons\%1.paa",_icon];
		};
